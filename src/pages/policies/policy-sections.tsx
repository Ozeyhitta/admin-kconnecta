...
export const SECTION_META: {
  ...
  { key: "admin_setup", label: "Thiết lập admin", icon: UserCheck },
];

export const AdminSetupSection = () => {
  ...
  const fetchAdmins = async () => {
    ...
    try {
      const [resAdmin, resSuper] = await Promise.all([
        apiClient.get("/api/v1/admin/users", { params: { role: "ADMIN", size: 100 } }),
        apiClient.get("/api/v1/admin/users", { params: { role: "SUPER_ADMIN", size: 100 } }),
      ]);
      const list = [...(resAdmin.data.content || []), ...(resSuper.data.content || [])];
  ...
  const handleCreateAdmin = async (e: FormEvent) => {
    ...
    setCreating(true);
    try {
      await apiClient.post("/api/v1/admin/users/admins", {
        email: email.trim(),
        password: password.trim(),
      });
      notify("Đã tạo tài khoản Admin thành công", { type: "success" });
      setEmail("");
      setPassword("");
      fetchAdmins();
    } catch (err: any) {
      const msg = err.response?.data?.message || "Lỗi khi tạo tài khoản Admin";
  ...
  const handleToggleStatus = async (adminAccount: any) => {
    if (currentUser?.id === adminAccount.id) {
      notify("Không thể tự thay đổi trạng thái của bản thân", { type: "warning" });
      return;
    }
    const newStatus = adminAccount.status === "ACTIVE" ? "BLOCKED" : "ACTIVE";
    try {
      await apiClient.patch(`/api/v1/admin/users/${adminAccount.id}/status`, {
        status: newStatus,
        lockDays: null,
      });
      notify(`Đã chuyển đổi trạng thái tài khoản thành ${newStatus}`, { type: "success" });
      fetchAdmins();
    } catch (err: any) {
  ...
  const handleToggleRole = async (adminAccount: any) => {
    if (currentUser?.id === adminAccount.id) {
      notify("Không thể tự đổi vai trò của bản thân", { type: "warning" });
      return;
    }
    const newRole = adminAccount.role === "SUPER_ADMIN" ? "ADMIN" : "SUPER_ADMIN";
    try {
      await apiClient.patch(`/api/v1/admin/users/${adminAccount.id}/role`, {
        role: newRole,
      });
      notify(`Đã chuyển vai trò tài khoản thành ${newRole}`, { type: "success" });
      fetchAdmins();
    } catch (err: any) {
      const msg = err.response?.data?.message || "Lỗi khi cập nhật vai trò";
      notify(msg, { type: "error" });
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center space-y-4">
        ...
        <div className="space-y-1 max-w-sm">
          <h3 className="text-sm font-semibold">Quyền truy cập bị từ chối</h3>
          <p className="text-xs text-muted-foreground">
            Chỉ Super Admin mới có quyền truy cập tab thiết lập và quản lý danh sách tài khoản Admin nội bộ.
          </p>
        </div>
      </div>
    );
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <div className="md:col-span-1">
          <PolicyCard
            ...
            description="Đăng ký thêm tài khoản quản trị viên mới hoạt động trong hệ thống"
          >
        ...
        <div className="md:col-span-2">
          <PolicyCard
            ...
            description="Tất cả các tài khoản Admin & Super Admin hiện tại trên hệ thống"
          >
            {loadingAdmins ? (
              ...
              <p className="text-xs text-muted-foreground text-center py-12 italic">
              ...
              <div className="rounded-md border border-border overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse text-left text-xs">
                    ...
                    <tbody className="divide-y">
                      {admins.map((ad) => {
                        const isSelf = currentUser?.id === ad.id;
                        return (
                          <tr key={ad.id} className={isSelf ? "bg-primary/5" : ""}>
                            <td className="p-3 font-medium">
                              {ad.email} {isSelf && <span className="text-[10px] text-primary">(Bạn)</span>}
                            </td>
                            <td className="p-3">
                              <Badge variant={ad.role === "SUPER_ADMIN" ? "destructive" : "default"}>
                                {ad.role}
                              </Badge>
                            </td>
                            <td className="p-3">
                              ...
                              {!isSelf && (
                                <>
                                  <Button
                                    ...
                                    onClick={() => handleToggleRole(ad)}
                                    title="Đổi vai trò ADMIN <-> SUPER_ADMIN"
                                  >
                                    Đổi Role
                                  </Button>
                                  <Button
                                    variant={ad.status === "ACTIVE" ? "destructive" : "outline"}
                                    size="sm"
                                    className="h-7 px-2 text-[11px]"
                                    onClick={() => handleToggleStatus(ad)}
                                  >
                                    {ad.status === "ACTIVE" ? "Khóa" : "Kích hoạt"}
                                  </Button>
                                </>
                              )}
                            </td>
