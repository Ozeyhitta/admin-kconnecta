...
import { useShowController, useUpdate, useNotify, useRefresh } from "ra-core";
import { useNavigate } from "react-router";
import {
  ShieldCheck, User, FileText, MessageSquare, Heart,
  Share2, UserPlus2, LogIn, Activity, ArrowLeft,
  CheckCircle, XCircle, Lock, KeyRound, TrendingUp,
} from "lucide-react";
...
const StatItem = ({
  ...
  icon: React.FC<any>; iconColor: string; label: string; value: number | undefined; loading: boolean;
}) => (
...
const ResetPasswordDialog = ({
  ...
  const handleSubmit = async () => {
    if (pw.length < 6) { notify("Mật khẩu phải có ít nhất 6 ký tự", { type: "warning" }); return; }
    setSaving(true);
    try {
...
export const CustomerShow = () => {
  ...
  const handleStatusChange = async (status: string) => {
    ...
    try {
      await update("customers", { id: record.id, data: { status }, previousData: record }, { returnPromise: true });
  ...
  const handleRoleChange = async (role: string) => {
    ...
    try {
      await update("customers", { id: record.id, data: { role }, previousData: record }, { returnPromise: true });
              ...
              <div className="space-y-1">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="LOCKED">Khoá tài khoản</SelectItem>
                  </SelectContent>
                </Select>
              </div>
          ...
          <Card>
            <CardContent className="pt-4">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <StatItem icon={TrendingUp} iconColor="text-violet-500" label="Engagement"
                  value={stats && stats.totalActivity > 0
