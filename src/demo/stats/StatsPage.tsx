...
const DauTrendChart = ({ data }: { data: { day: string; count: number }[] | null }) => {
...
const InteractionChart = () => {
  ...
  React.useEffect(() => {
    setData(null);
    apiClient
      .get<DataPoint[]>("/api/v1/admin/stats/interactions", { params: { period } })
...
const StatsPage = () => {
  ...
  React.useEffect(() => {
    apiClient
      .get<DauMauData>("/api/v1/admin/stats/dau-mau")
      .then(r => setDauMau(r.data))
      .catch(() => setDauMau(null))
        ...
        <section>
          ...
          <Card>
            <CardContent className="pt-4">
              ...
              <DauTrendChart data={loading ? null : (dauMau?.dauTrend ?? [])} />
            </CardContent>
          </Card>
        </section>
